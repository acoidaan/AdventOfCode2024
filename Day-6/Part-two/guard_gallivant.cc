...
public:
    ...
    Path GetOriginalPath() {
        std::vector<std::vector<char>> pattern(rows_, std::vector<char>(cols_, '.'));
        std::set<Point> points;
        Point pos = start_;
    ...
    int CountLoopPoints() {
        ...
        for (const auto& point : originalPath.points) {
            for (int dir = 0; dir < 4; ++dir) {
                ...
                if (IsInBounds(adj) && map_[adj.row][adj.col] == '.' && 
                    adj != start_ && originalPath.points.find(adj) == originalPath.points.end()) {
